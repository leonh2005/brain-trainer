...
### 憑證遮蔽強化（2026-09-24，依 Steven 要求）

Steven 明確表示：隱私整體無所謂，**但密碼那類憑證絕對不能洩漏**。因此重點從「去識別化」轉為「憑證零外洩」。

**用真實資料實測**（937 個 tool_result、87 萬字元）：掃描 `redact()` 之後的內容，用 18 種憑證樣式找漏網——**零實際洩漏**。命中的 12 筆全是誤報（已遮蔽的標記、`open(...secrets/...)` 這類讀取程式碼、以及一個不完整的 bot id 片段（只有數字前綴，後面沒有完整 token））。

**仍補強了 review 點出的缺口**（現在沒出現不代表以後不會）：

| 新增樣式 | 例子 |
|---|---|
| Basic auth | `Authorization: Basic YWRtaW46...` |
| curl 帳密 | `curl -u admin:hunter2` |
| 連線字串密碼 | `postgres://user:S3cretPwd@host` |
| AWS secret key | 40 字元 base64（需上下文辨識） |
| 大寫常數賦值 | `FINMIND_TOKEN=...`、`BOT_TOKEN=...` |
| 通用賦值的值允許 `:` | token 常長成 `1234:AAH...` |

另修正一個順序問題：連線字串規則原本排在 email 之後，導致密碼被標成 `[REDACTED_EMAIL]`（安全但標籤誤導）。

驗證方式：21 個合成憑證測資**全數通過**、真實資料只減少 0.08%（沒有過度遮蔽）。測試總數 **36 個**。
...
### 2. 走哪個 provider？　✅ 已定：A（直連 TypeSafe）

Steven 2026-09-24 表示「隱私倒是無所謂」→ 不需要為了 ZDR 去開 Vercel Pro。維持直連現有 key。
（B 的實作路徑仍保留在 `jev_client.py` 的 `base_url` 參數，日後要換再換。）

### 3. 偽名化要不要預設開啟？　✅ 已定：關閉

Steven 只要求「密碼那些不能洩漏」，不在意對話內容去識別化。
偽名化改為**預設關閉**（`JEV_COMPACTION_ANON=1` 可開啟），理由是它會微幅干擾判斷（實測一致率 98.6%）且多耗 CPU，既然沒有隱私需求就沒必要開。

**⚠️ 憑證遮蔽（redact）不受此開關影響，永遠開啟。**
